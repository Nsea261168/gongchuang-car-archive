# 工创小车差速方案（项目归档）

这是工创赛智能救援小车项目停止开发后的公开归档。仓库保留目前可用的 STM32 主控固件和整车 Fusion 360 装配体，全部固件均由Chatgpt-5.6-sol开发，建模使用fusion360完成。

> 项目状态：因队内协调以及其他各种原因，我小队弃赛，现将本人开发部分全部开源，目前本项目已停止维护。现有设计和代码已完成全部实车验证，可正常过减速带，推动所有物块，仅供后来者参考。

## 仓库内容

| 目录 | 内容 |
| --- | --- |
| `firmware/stm32h7/` | 基于 STM32H723 的智能救援车主控固件、Keil 工程和开发文档 |
| `mechanical/工创小车差速方案.f3z` | Autodesk Fusion 360 整车装配体归档 |
| `mechanical/工创小车麦克纳姆.f3z` | 第一版四驱麦克纳姆轮整车装配体；因整车超重而放弃，未进行实装测试 |

## 固件快速入口

- Keil 工程：`firmware/stm32h7/MDK-ARM/CtrBoard-H7_FDCAN.uvprojx`
- CubeMX 配置：`firmware/stm32h7/CtrBoard-H7_FDCAN.ioc`
- 开发说明：`firmware/stm32h7/Docs/DEVELOPMENT_GUIDE.md`
- 项目架构：`firmware/stm32h7/Docs/PROJECT_ARCHITECTURE.md`
- 项目规则：`firmware/stm32h7/Docs/PROJECT_RULES.md`

固件面向达妙 STM32H723 控制板，采用裸机协作式调度。项目包含 CAN/FDCAN、UART、IMU、激光雷达、底盘电机、舵机、LCD、按键和任务状态机等模块。具体完成度、硬件约束和安全要求请以 `firmware/stm32h7/Docs/` 中的文档为准。
由于时间原因，本项目所有硬件均为成品，目的为快速验证原型，根据工创赛比赛规则，各位应自主涉及硬件。电机使用mg513，65mm橡胶轮，驱动板使用松甲科技四路驱动板，激光雷达为ld06。松甲科技驱动板可以直插jdy31蓝牙模块便捷的遥控，测试机械结构可行性。

## 使用整车装配体

`.f3z` 是 Fusion 360 归档格式。可在 Autodesk Fusion 中通过“文件 → 打开/上传”导入。模型约 50 MB，本仓库使用 Git LFS 管理该文件；通过 Git 克隆仓库时请确保已安装 Git LFS。

其中麦克纳姆轮方案是项目早期废案。该方案因整车重量超出预期而未进入实装测试，仅用于展示设计思路和方案演进，不建议未经重新核算重量、强度、重心及驱动能力就直接加工使用。

```bash
git lfs install
git clone https://github.com/Nsea261168/gongchuang-car-archive.git
```

## 重要说明

- 由于机械结构、硬件测试以及电控全部由我一人完成，时间仓促，以及本人能力有限，fusion草图定义，固件工程结构，数据调度均存在一定问题，还请大家谅解，指出问题所在，共同进步。
- 本项目属于未完成工程归档，不保证可以直接参赛、量产或安全运行。
- 上电及实车测试前，应先架空车轮，确认急停、通信超时、方向和速度限制有效。
- 仓库公开不自动改变各文件原有的许可状态。固件目录保留了原仓库中的 `LICENSE` 以及第三方组件许可证；历史文档对许可证的描述存在不一致，实际复用前请自行核对权利归属。
- 整车装配体目前没有单独声明许可；未经权利人另行授权，不应默认获得商业使用权。
- 如有任何问题，可向本人邮箱501621243@qq.com致信 或添加本人qq交流

## 致后来者

- 对于工创赛，我抱着极大的热情，先后设计了多种方案，结构上有四驱麦轮，定位上有惯性导航，深度相机等等，最后采用了成本较低且性能较好的激光雷达定位，单摄像头运行yolo的方案，不过现在看来，也许双tof激光测距正交加上陀螺仪数据定位是比较好的选择。
- 这个项目没有走到最后，但留下的每一行代码、每一次结构调整和每一个没有验证完的方案，都可能成为后来者少走的一段弯路。欢迎阅读、研究并在明确许可范围内继续完善它，如果能帮到大家，那么我的心血也算没有白流。
